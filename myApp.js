...
var findEditThenSave = function (personId, done) {
  ...
  Person.find(personId, function (err, personArrayResult) {

    if (err) return done(err);

    if (personArrayResult && personArrayResult.length) {
      personArrayResult.forEach(function (person) {
...
var findAndUpdate = function (personName, done) {
  ...
  var newValue = { age: 20 };
  Person.findOneAndUpdate(query, newValue, { new: true }, function (err, person) {
    if (err) return done(err);
    done(null, person);
  });
};
...
var removeManyPeople = function (done) {
  ...
  Person.remove({ name: nameToRemove }, function (err, doc, removed) {
    if (err) return done(err);
    done(null, doc, removed);
  });
};
...
var queryChain = function (done) {
  ...
  Person
    ...
    .sort({ name: 1 })
    .select({ name: 1, body:1, favoriteFoods:1 })
    .limit(2)
